...
export default function ZoneComplaintChart({ data = [] }) {
  const [tooltip, setTooltip] = useState(null);

  if (!data || data.length === 0) {
    return (
      <ChartCard title="Zone-wise Open Complaints">
        <div style={{ textAlign: 'center', padding: '30px', color: '#777' }}>No open complaints for selected zone.</div>
      </ChartCard>
    );
  }

  // Find maximum total open complaints for scaling single stacked vertical bars
  ...
  return (
    <ChartCard title="Zone-wise Open Complaints">
      ...
      {/* Increased Height Canvas (360px) with Single Stacked Bar Per Zone */}
      <div style={{ position: 'relative', height: '360px', display: 'flex', alignItems: 'flex-end', gap: '22px', paddingBottom: '35px', borderBottom: '2px solid #e5e7eb', overflowX: 'auto' }}>
        {data.map((item) => {
          ...
          return (
            <div key={item.zone_code} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, width: '48px' }}>
              {/* Single Stacked Vertical Bar per Zone */}
              <div style={{ display: 'flex', flexDirection: 'column', width: '32px', height: `${chartHeightPx}px`, justifyContent: 'flex-end' }}>
                ...
                {item.high > 0 && (
                  <div
                    onMouseEnter={(e) => handleMouseEnter(e, `${item.zone_name} — High: ${item.high} Open Complaints`)}
                ...
                {item.medium > 0 && (
                  <div
                    onMouseEnter={(e) => handleMouseEnter(e, `${item.zone_name} — Medium: ${item.medium} Open Complaints`)}
                    onMouseLeave={handleMouseLeave}
                    style={{
                      height: `${medH}px`,
                ...
                {item.low > 0 && (
                  <div
                    onMouseEnter={(e) => handleMouseEnter(e, `${item.zone_name} — Low: ${item.low} Open Complaints`)}
                    onMouseLeave={handleMouseLeave}
                    style={{
                      height: `${lowH}px`,
